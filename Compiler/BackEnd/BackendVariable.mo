...
public function isJacobianVar
"Return true if variable generated by symbolic jacobian and is helper variable"
  input BackendDAE.Var inVar;
  output Boolean outBoolean;
algorithm
  outBoolean := match (inVar)
    case (BackendDAE.VAR(varKind =  BackendDAE.JAC_VAR())) then true;
    else false;
  end match;
end isJacobianVar;

public function isJacobianDiffVar
"Return true if variable generated by symbolic jacobian and is desired derivative variable"
  input BackendDAE.Var inVar;
  output Boolean outBoolean;
algorithm
  outBoolean := match (inVar)
    case (BackendDAE.VAR(varKind =  BackendDAE.JAC_DIFF_VAR())) then true;
    else false;
  end match;
end isJacobianDiffVar;
...
algorithm
  (v_lst, _) := traverseBackendDAEVars(inVariables,traversingVarsOnTrue,({}, inFunc));
end getVarsOntrue;

protected function traversingVarsOnTrue
  input BackendDAE.Var inVar;
  input tuple<list<BackendDAE.Var>, CompareFunc> inTpl;
  output BackendDAE.Var outVar = inVar;
  output tuple<list<BackendDAE.Var>, CompareFunc> outTpl;
partial function CompareFunc
  input BackendDAE.Var inVar;
  output Boolean outBoolean;
end CompareFunc;
...
algorithm
  (varLst, compFunc) := inTpl;
  outTpl := if compFunc(inVar) then (inVar::varLst, compFunc) else inTpl; 
end traversingVarsOnTrue;

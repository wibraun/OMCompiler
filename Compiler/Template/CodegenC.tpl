...
::=
  ...
  case simCode as SIMCODE(modelInfo=MODELINFO(functions=functions)) then
    ...
    extern
           ...
           int <%symbolName(modelNamePrefixStr,"copy_ADOLC_dep")%>(DATA *data, threadData_t *threadData, double* dependentVars);
    <%\n%>
    >>
  end match
...
::=
  ...
  let fncalls = if Flags.isSet(Flags.PARMODAUTO) then
                ...
                let &eqfuncs += (initalEquations |> eq hasindex i0 =>
                    equation_impl(-1, eq, contextSimulationDiscrete, modelNamePrefix)
                    ;separator="\n")
                (initalEquations |> eq hasindex i0 => equation_call(eq, modelNamePrefix) ;separator="\n")
...
::=
  ...
  int <%symbolName(modelNamePrefix,"functionJac")%><%matrixName%>_column(void* inData, threadData_t *threadData)
  {
    ...
    int index = <%symbolName(modelNamePrefix,"INDEX_JAC_")%><%matrixName%>;
    <%(jacEquations |> eq =>
    equation_call(eq, modelNamePrefix); separator="\n")%>
...
case eq as SES_SIMPLE_ASSIGN(__) then
  ...
  let lhsPre =  match context case ADOLC_CONTEXT() then 'ad' else ''
  <<
...
::=
  match simVar
  case SIMVAR(aliasvar=NOALIAS()) then 'ad<%cref(name)%>  <<=  <%cref(name)%>;'
  end match
  ...
  match modelInfo
    case MODELINFO(varInfo=VARINFO(numStateVars=numStateVars), vars=SIMVARS(__),name=name) then
     let modelNamePrefixStr = underscorePath(name)
      ...
      void <%symbolName(modelNamePrefixStr,"copy_ADOLC_dep")%>(DATA* data, double* vars)
      {
        /* derivative vars */
        <%vars.derivativeVars |> var =>
...
::=
  let equations = (List.flatten(eqs) |> eq =>
    equation_impl(-1, eq, contextADOLC, modelNamePrefixStr); separator="\n")

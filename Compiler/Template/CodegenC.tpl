...
match sparsepattern
  ...
  case _ then
      ...
      let columnCalls = (jacobianColumn |> JAC_COLUMN(columnCalls=eqCalls) =>
                        match eqCalls case {} then
                          'NULL'
                        else
      ...
      let calleqn = "eqAttrCall_" + '<%matrixname%>'
      let eqAttrCalls = (jacobianColumn |> JAC_COLUMN(columnCalls=calls) =>
                            (calls |>  i_index =>
                               (i_index |> idx => '<%idx%>'; separator=", ");
                             separator=", ");
                         separator="")
      <<
      // Array encoding which equation needs to be evaluated for which color w.r.t. colord Jacobian evaluation
      static int eqAttrCall_<%matrixname%>[<%numEqs%>*<%maxColor%>] = {<%eqAttrCalls%>};
  ...
  case _ then
    match seedVars
     case {} then
      ...
      case _ then
        let jacMats =
        (jacobianColumn |> JAC_COLUMN(columnEqns=eqs, columnCalls=eqCalls, constantEqns=constantEqns) =>
          match eqCalls
          case {} then
            functionJac(eqs, constantEqns, partIdx, matrixname, jacHT, modelNamePrefix)
          case _ then
            functionJacDepCalls(eqs, eqCalls, constantEqns, partIdx, matrixname, jacHT, modelNamePrefix)
            ;separator="\n")
...
template functionJac(list<SimEqSystem> jacEquations, list<SimEqSystem> constantEqns, Integer partIdx, String matrixName, Option<HashTableCrefSimVar.HashTable> jacHT, String modelNamePrefix) "template functionJac
...
template functionJacDepCalls(list<SimEqSystem> jacEquations, list<list<Integer>> eqCalls, list<SimEqSystem> constantEqns, Integer partIdx, String matrixName, Option<HashTableCrefSimVar.HashTable> jacHT, String modelNamePrefix) "template functionJac
  This template generates functions for each column of a single jacobian.
  This is a helper of generateMatrix."
::=
  let constantEqns2 = generateConstantEqns(constantEqns, matrixName, modelNamePrefix)
  ...
  OMC_DISABLE_OPT
  int <%symbolName(modelNamePrefix,"functionJac")%><%matrixName%>_columnColor(void* inData, threadData_t *threadData, int color, ANALYTIC_JACOBIAN *jacobian, ANALYTIC_JACOBIAN *parentJacobian)
  {
    TRACE_PUSH
    ...
    int i = 0;
    <%(jacEquations |> eq => '<%functionJacDepCallsGenerateIfStat(eq, modelNamePrefix, matrixName)%>'; separator="\n")%>
...
template functionJacDepCallsGenerateIfStat(SimEqSystem eq, String modelNamePrefix, String matrixName)
"MS: jacIdx kann raus, weil im Code schon index = ... steht und wir dann hier einfach die Variable
     Index nutzen können.
"
::=
  let idx = equationIndex(eq)
  let jacIdx = jacobianIndex(modelNamePrefix, matrixName)
  <<
  if (1 == data->simulationInfo->analyticJacobians[<%jacIdx%>].callEqns[i++*color])
    <%equation_callJacobian(eq, modelNamePrefix)%>
  >>
end functionJacDepCallsGenerateIfStat;

template jacobianIndex(String modelNamePrefix, String matrixName)
"Return index of Jacobian."
::=
  <<
  <%symbolName(modelNamePrefix,"INDEX_JAC_")%><%matrixName%>
  >>
end jacobianIndex;

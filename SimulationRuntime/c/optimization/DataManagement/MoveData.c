...
void diffSynColoredOptimizerSystem(OptData *optData, modelica_real **J, const int m, const int n, const int index){
  ...
  for(i = 1; i < Cmax; ++i){
    ...
    if(index == 2){
      data->callback->functionJacB_column(data, threadData, jacobian);
    }else if(index == 3){
      data->callback->functionJacC_column(data, threadData, jacobian);
    }else
      assert(0);
...
void diffSynColoredOptimizerSystemF(OptData *optData, modelica_real **J){
  if(optData->dim.ncf > 0){
    ...
    for(i = 1; i < Cmax; ++i){
      ...
      data->callback->functionJacD_column(data, threadData, jacobian);
